...
type ListEmployeesAdminParams struct {
	BarberShopID uuid.UUID `json:"barber_shop_id"`
...
func (q *Queries) ListEmployeesAdmin(ctx context.Context, arg ListEmployeesAdminParams) ([]ListEmployeesAdminRow, error) {
	rows, err := q.db.Query(ctx, listEmployeesAdmin, arg.BarberShopID, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListEmployeesAdminRow{}
	for rows.Next() {
