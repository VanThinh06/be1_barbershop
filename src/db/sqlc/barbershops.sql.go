...
func (q *Queries) GetBarberShop(ctx context.Context, id uuid.UUID) (BarberShop, error) {
	row := q.db.QueryRow(ctx, getBarberShop, id)
	var i BarberShop
	err := row.Scan(
		&i.ID,
		&i.BarberShopChainID,
...
func (q *Queries) GetDefaultPasswordEmployee(ctx context.Context, id uuid.UUID) (string, error) {
	row := q.db.QueryRow(ctx, getDefaultPasswordEmployee, id)
	var default_employee_password string
