...
const listCategoryPosition = `-- name: ListCategoryPosition :many
SELECT 
...
type ListCategoryPositionRow struct {
	ID           int16         `json:"id"`
	Name         string        `json:"name"`
	BarberShopID uuid.NullUUID `json:"barber_shop_id"`
...
func (q *Queries) ListCategoryPosition(ctx context.Context, barberShopID uuid.NullUUID) ([]ListCategoryPositionRow, error) {
	rows, err := q.db.Query(ctx, listCategoryPosition, barberShopID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListCategoryPositionRow{}
	for rows.Next() {
		var i ListCategoryPositionRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.BarberShopID,
