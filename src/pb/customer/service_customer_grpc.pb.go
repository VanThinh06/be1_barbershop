...
func (c *customerBarberShopClient) GetAppointmentByDate(ctx context.Context, in *GetAppointmentByDateRequest, opts ...grpc.CallOption) (*GetAppointmentByDateResponse, error) {
	out := new(GetAppointmentByDateResponse)
	err := c.cc.Invoke(ctx, CustomerBarberShop_GetAppointmentByDate_FullMethodName, in, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func _CustomerBarberShop_GetAppointmentByDate_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(GetAppointmentByDateRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(CustomerBarberShopServer).GetAppointmentByDate(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: CustomerBarberShop_GetAppointmentByDate_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(CustomerBarberShopServer).GetAppointmentByDate(ctx, req.(*GetAppointmentByDateRequest))
	}
	return interceptor(ctx, in, info, handler)
}
