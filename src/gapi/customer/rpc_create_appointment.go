...
func (server *Server) NewAppointment(ctx context.Context, req *customer.CreateAppointmentRequest) (*customer.CreateAppointmentResponse, error) {
	...
	res, err := server.store.CreateAppointment(ctx, arg)
	if err != nil {
	...
	message := &messaging.Message{
		Data: map[string]string{
			...
			"time":                 "2:45",
			"message":              "Hello Work",
			"appointment_datetime": res.AppointmentDatetime.Format("2006-01-02T15:04:05Z"),
		},
	...
	rsp := &customer.CreateAppointmentResponse{
		Appointment: convertAppointment(res),
	}
	return rsp, nil
}
