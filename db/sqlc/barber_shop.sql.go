...
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING shop_id, code_barber_shop, owner_id, name, location, address, image, list_image, status, created_at, update_at
`
...
SELECT shop_id, code_barber_shop, owner_id, name, location, address, image, list_image, status, created_at, update_at
FROM "BarberShops"
WHERE code_barber_shop = $1
LIMIT 1
`

func (q *Queries) GetBarberShop(ctx context.Context, codeBarberShop string) (BarberShop, error) {
	row := q.db.QueryRowContext(ctx, getBarberShop, codeBarberShop)
	var i BarberShop
	err := row.Scan(
		&i.ShopID,
		&i.CodeBarberShop,
